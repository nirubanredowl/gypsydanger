...
On a remote machine without a browser:

```bash
aws login --remote
```

If your org uses SSO instead, use your existing `aws sso login --profile …` workflow.
...
Uses the official [AWS API MCP Server](https://github.com/awslabs/mcp/tree/main/src/aws-api-mcp-server) (`awslabs.aws-api-mcp-server`) via `uvx`.
