...
def recommendation_block(stats: ScanStats) -> list[str]:
    ...
    return [
        ...
        "",
        "### Tier B — edge cases (typed tag, weak headline)",
        f"- **~{tier_b:,} rows** — e.g. `CFO Update` with `Company Secretary Appointment/Resignation`.",
        "- **554 Tier-A rows** also carry `Director Appointment/Resignation` or similar typed tags.",
        "- ~107 `Director Appointment/Resignation` rows mention CFO in the headline (most are Tier A).",
        "",
