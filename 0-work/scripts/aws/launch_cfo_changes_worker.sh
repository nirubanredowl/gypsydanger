#!/usr/bin/env bash
# Launch one CFO change fetch worker EC2 instance.
set -euo pipefail
...
INCLUDE_TIER_B="${GYPSY_CFO_INCLUDE_TIER_B:-0}"
TIER_B_FLAG=""
if [[ "$INCLUDE_TIER_B" == "1" ]]; then
  TIER_B_FLAG="--include-tier-b"
fi
LOG_PREFIX="logs/cfo_fetch/${RUN_ID}"
shard_id="$(printf '%02d' "$WORKER_ID")"
name="gypsy-danger-cfo-fetch-w${shard_id}"
...
mkdir -p "\$ROOT/0-work/scripts" "\$ROOT/data/cfo_changes"
...
aws s3 cp "s3://${BUCKET}/scripts/16_fetch_cfo_changes_s3.py" "\$ROOT/0-work/scripts/"
aws s3 cp "s3://${BUCKET}/scripts/18_fetch_cfo_changes_shard.py" "\$ROOT/0-work/scripts/"
aws s3 cp "s3://${BUCKET}/cfo_changes/shards/${WORKERS}workers/shard_${shard_id}.txt" "\$ROOT/data/cfo_changes/shard.txt"
cd "\$ROOT/0-work/scripts"
set +e
python3 18_fetch_cfo_changes_shard.py \\
  --tickers-file "\$ROOT/data/cfo_changes/shard.txt" \\
  --bucket ${BUCKET} \\
  --run-id ${RUN_ID} \\
  --worker-id ${shard_id} \\
...
IID="$(aws ec2 run-instances \
  ...
  --tag-specifications \
    "ResourceType=instance,Tags=[{Key=Name,Value=${name}},{Key=Project,Value=gypsy-danger},{Key=Application,Value=gypsy-danger-asx-fetch},{Key=Stage,Value=cfo-fetch},{Key=CfoFetchRunId,Value=${RUN_ID}},{Key=CfoFetchWorkerId,Value=${shard_id}},{Key=CfoFetchRotation,Value=${ROTATION}},{Key=ManagedBy,Value=gypsy-danger-cfo-fetch}]" \
...
print(json.dumps({
    ...
    "workers": int("${WORKERS}"),
    "include_tier_b": bool(int("${INCLUDE_TIER_B}")),
}))
PY
)"
