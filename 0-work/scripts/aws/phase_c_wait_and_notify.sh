#!/usr/bin/env bash
# Poll Phase C worker results, rotate burned EC2s, update progress manifest, email on completion.
set -euo pipefail
...
TOTAL_PDFS="${GYPSY_TOTAL_PDFS:-22573}"
PREFIX="logs/fetch/${RUN_ID}/"
LAUNCH="${SCRIPT_DIR}/launch_phase_c_worker.sh"
if [[ ! -x "$LAUNCH" ]]; then
  ROOT="/opt/gypsy-danger"
  mkdir -p "$ROOT"
  aws s3 cp "s3://${BUCKET}/scripts/launch_phase_c_worker.sh" "$LAUNCH"
  chmod +x "$LAUNCH"
fi

WAIT_MAX_S="${GYPSY_FETCH_WAIT_MAX_S:-86400}"
DEADLINE=$(( $(date +%s) + WAIT_MAX_S ))
POLL_S="${GYPSY_FETCH_POLL_S:-60}"
echo "Phase C waiter: run=${RUN_ID} workers=${WORKERS} prefix=s3://${BUCKET}/${PREFIX} max_wait=${WAIT_MAX_S}s"
...
total = int("${TOTAL_PDFS}")
status = "running"
if complete_workers >= int("${WORKERS}") and all(r.get("complete") for r in rows if len(rows) >= int("${WORKERS}")):
    status = "complete" if pdfs_done >= total * 0.99 else "running"
if pdfs_done >= total:
    status = "complete"

manifest_path = Path("/tmp/fetch_progress.json")
existing_raw = subprocess.run(
    ["aws", "s3", "cp", f"s3://${BUCKET}/manifests/fetch_progress.json", "-", "--no-cli-pager"],
    capture_output=True,
    text=True,
...
PY
  aws s3 cp /tmp/fetch_progress.json "s3://${BUCKET}/manifests/fetch_progress.json"
  rm -rf "$TMPDIR"
}
...
summary = {
    ...
    "annual_filter": "loose",
    "pdfs_total": int("${TOTAL_PDFS}"),
    "passed": passed,
    "s3_pdf_prefix": f"s3://${BUCKET}/entities/",
    "s3_logs_prefix": f"s3://${BUCKET}/${PREFIX}",
}
print(json.dumps(summary, indent=2))
Path("/tmp/fetch-summary.json").write_text(json.dumps(summary, indent=2) + "\n")
PY
)"

echo "$SUMMARY"
aws s3 cp /tmp/fetch-summary.json "s3://${BUCKET}/manifests/fetch/${RUN_ID}/summary.json"
update_progress_manifest || true

MSG="$(python3 - <<PY
import json
from pathlib import Path
s = json.loads(Path("/tmp/fetch-summary.json").read_text())
verdict = "PASS" if s["passed"] else "DONE"
lines = [
    f"Gypsy Danger Phase C fetch — {verdict}",
    "",
    f"Run ID:            {s['run_id']}",
    ...
    f"Annual filter:     {s['annual_filter']} (~{s['pdfs_total']:,} reports)",
    "",
    f"S3 PDFs:   {s['s3_pdf_prefix']}",
    f"S3 logs:   {s['s3_logs_prefix']}",
    f"Summary:   s3://${BUCKET}/manifests/fetch/{s['run_id']}/summary.json",
    f"Progress:  s3://${BUCKET}/manifests/fetch_progress.json",
]
print("\\n".join(lines))
PY
)"
...
if [[ -n "${GYPSY_SNS_TOPIC_ARN:-}" ]]; then
  VERDICT="$(python3 -c "import json; print('PASS' if json.load(open('/tmp/fetch-summary.json'))['passed'] else 'DONE')")"
  aws sns publish \
    --topic-arn "$GYPSY_SNS_TOPIC_ARN" \
    --subject "Gypsy Danger Phase C fetch — ${VERDICT}" \
    --message "$MSG"
...
IDS="$(aws ec2 describe-instances \
  --filters "Name=tag:Project,Values=gypsy-danger" "Name=tag:FetchRunId,Values=${RUN_ID}" \
    "Name=instance-state-name,Values=pending,running,stopping,stopped" \
  --query 'Reservations[].Instances[].InstanceId' --output text)"
if [[ -n "$IDS" && "$IDS" != "None" ]]; then
  echo "Terminating fetch instances: $IDS"
