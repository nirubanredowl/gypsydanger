#!/usr/bin/env bash
# Launch one Stage 3A LiteParse worker EC2 instance.
set -euo pipefail
...
INSTANCE_TYPE="${GYPSY_PARSE_INSTANCE_TYPE:-c6i.large}"
LOG_PREFIX="logs/parse/${RUN_ID}"
shard_id="$(printf '%02d' "$WORKER_ID")"
name="gypsy-danger-parse-w${shard_id}"
...
aws s3 cp "s3://${BUCKET}/scripts/23_liteparse_shard.py" "\$ROOT/0-work/scripts/"
...
aws s3 cp /tmp/parse.log "s3://${BUCKET}/${LOG_PREFIX}/worker_${shard_id}.log" || true
exit \$EXIT
USERDATA
)"
...
IID="$(aws ec2 run-instances \
  ...
  --tag-specifications \
    "ResourceType=instance,Tags=[{Key=Name,Value=${name}},{Key=Project,Value=gypsy-danger},{Key=Application,Value=gypsy-danger-asx-parse},{Key=Stage,Value=parse},{Key=ParseRunId,Value=${RUN_ID}},{Key=ParseWorkerId,Value=${shard_id}},{Key=ParseRotation,Value=${ROTATION}},{Key=ManagedBy,Value=gypsy-danger-parse}]" \
