#!/usr/bin/env python3
"""Fetch CFO change PDFs for a ticker shard and upload to S3."""
...
cfo_fetch = import_module("16_fetch_cfo_changes_s3")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-id", default=os.environ.get("GYPSY_CFO_FETCH_RUN_ID", ""))
    parser.add_argument(
        "--worker-id",
    ...
    parser.add_argument("--include-tier-b", action="store_true")
...
def shard_doc_count(
    ...
    *,
    ...
    for idx in range(from_index, len(tickers)):
        ...
        matched = cfo_fetch.cfo_change_rows(
            rows, include_tier_b=include_tier_b, start_offset=0
        )
        if idx == from_index:
...
def build_payload(
    *,
    ...
    return {
        ...
        "complete": complete,
        "bucket": args.bucket,
        "s3_prefix": "entities/{TICKER}/cfo_changes/",
    }
...
def main() -> int:
    ...
    print(
        f"CFO shard: worker={args.worker_id} rotation={args.rotation} "
        f"tickers={len(tickers)} resume_index={args.ticker_index} "
        f"start_offset={args.start_offset} targets={docs_target} "
        f"tier={'A+B' if args.include_tier_b else 'A'} bucket={args.bucket}"
    )
    ...
    for idx in range(args.ticker_index, len(tickers)):
        ...
        targets = cfo_fetch.cfo_change_rows(
            rows, include_tier_b=args.include_tier_b, start_offset=offset
        )
        ...
        for row in targets:
            ...
            if burned:
                ...
                print(
                    f"    BURNED at ticker={ticker} resume_offset={resume_offset}"
                )
                snap = burn.snapshot()
                upload_progress(
